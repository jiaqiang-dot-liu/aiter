...
#pragma unroll
    ...
    // Aggregate only valid partitions.  The old chunked implementation issued
    // 16, 32, or 64 tmp_out loads per head element and redirected every invalid
    // lane to the final valid partition.  Decode commonly has far fewer than
    // 16 partitions, so those redundant global reads amplified workspace
    // traffic.  Keeping a single accumulator also removes the 64-element
    // per-thread temporary array and its attendant register/local-memory cost.
